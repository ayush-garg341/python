...
def recursive_brute_force(
    profits: list, weights: list, capacity: int, current_idx: int
) -> int:
    ...
    if weights[current_idx] <= capacity:
        profit1 = profits[current_idx] + recursive_brute_force(
            profits, weights, capacity - weights[current_idx], current_idx
        )
...
def unbounded_knapsack_dp_top_bottom(
    profits: list, weights: list, capacity: int, dp: list, current_idx: int
) -> int:
    if current_idx >= len(profits):
        return 0
    ...
    if dp[current_idx][capacity] == 0:
        profit1 = 0
        if weights[current_idx] <= capacity:
            profit1 = profits[current_idx] + unbounded_knapsack_dp_top_bottom(
                profits, weights, capacity - weights[current_idx], dp, current_idx
            )

        profit2 = unbounded_knapsack_dp_top_bottom(
            profits, weights, capacity, dp, current_idx + 1
        )
