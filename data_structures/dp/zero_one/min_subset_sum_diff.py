...
def min_subset_sum_diff_brute_force_recursive(
    arr: list, current_idx: int, find: int
) -> int:
    ...
    if arr[current_idx] <= find:
        sum1 = arr[current_idx] + min_subset_sum_diff_brute_force_recursive(
            arr, current_idx + 1, find - arr[current_idx]
        )
...
def min_subset_sum_diff_top_bottom_dp(
    arr: list, current_idx: int, find: int, dp: list
) -> int:
    if find == 0:
        return 0
    if current_idx >= len(arr):
    ...
    sum1 = 0
    if arr[current_idx] <= find:
        sum1 = arr[current_idx] + min_subset_sum_diff_top_bottom_dp(
            arr, current_idx + 1, find - arr[current_idx], dp
        )
...
print(
    "======================dp top bottom using two subset sum========================="
)
