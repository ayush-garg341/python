...
def can_partition_recursive_with_memoization(
    nums: list, current_idx: int, dp: list, s: float
) -> int:
    ...
    if dp[current_idx][s] == -1:
        if nums[current_idx] <= s:
            if can_partition_recursive_with_memoization(
                nums, current_idx + 1, dp, s - nums[current_idx]
            ):
                dp[current_idx][s] = 1
                return 1

        dp[current_idx][s] = can_partition_recursive_with_memoization(
            nums, current_idx + 1, dp, s
        )
