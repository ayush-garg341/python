"""
Switch Interval:- Python allows us to tweak the switch interval, which is the ideal thread switching delay inside the interpreter. It is the time interval after which the Python interpreter schedules another thread for execution. If we reduce it to a very small value, say 2 milliseconds, we should see more number of requests being completed even with the long-running task. The switch interval can be controlled using the API sys.setswitchinterval().
"""
...
"""
Unfortunately, even if we have more than one processor on the machine running our program, the additional CPUs can't lend us a helping hand because of GIL.
"""
